    ...
    const startAnalysis = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // analysisResult에 있는 dangerousCommands를 사용하여 steps를 생성
          // 
          setSteps([
            ...steps,
            { text: `> { ${dangerousCommands.map((cmd:any) => cmd.dangerous).join(", ")} } is detected in { ${file.name} }`, className: "text-red-400 mb-2 text-center", delay: 1500 },
            { text: `> Those should be changed to { ${dangerousCommands.map((cmd:any) => cmd.safe).join(", ")} }`, className: "text-yellow-400 mb-4 text-center", delay: 1500 },
            { text: `> Continue to Download Patched { ${file.name} } ?`, className: "text-green-400 mb-2 text-center", delay: 1000 },
          ])
        }
      } catch (err) {
